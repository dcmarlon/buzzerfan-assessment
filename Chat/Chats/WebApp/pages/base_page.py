# Chat/Chats/WebApp/pages/base_page.py
"""Base page object.
...
class BasePage:
    ...
    def is_visible(self, locator: Locator, timeout: int | None = None) -> bool:
        ...
        Used for assertions without raising when the element is legitimately
        absent.
    ...
    def wait_present(self, locator: Locator, timeout: int | None = None) -> bool:
        """Return True if at least one matching element is present in the DOM.

        "Present" is weaker than "visible": it covers elements that exist but
        may not yet be painted. Used to detect whether a screen rendered, and
        whether a list has any rows, without raising.

        Args:
            locator: (By, value) tuple identifying the element(s).
            timeout: Optional override for this single check.

        Returns:
            True if present before the timeout, else False.
        """
        try:
            WebDriverWait(self.driver, timeout or self.timeout).until(
    ...
    def wait_invisible(self, locator: Locator, timeout: int | None = None) -> bool:
        ...
        Useful for confirming that a page has been left behind — e.g. the login
        form disappearing once authentication succeeds.
