# Auth/Registration/Android/utils/driver_factory.py
"""Appium driver factory and configuration loading.

Responsibilities:
- Load the static Appium settings from ``capabilities.json``.
- Load the per-machine values (APK path, test-account details) from
  ``credentials.json``.
...
# Feature root = Auth/Registration/Android (one level up from utils/).
...
# Repo root is three levels up: <repo>/Auth/Registration/Android. Relative APK
# paths in credentials.json are interpreted from here.
...
def load_credentials() -> dict[str, Any]:
    """Load the per-machine credentials (apk_path, new_user details).
