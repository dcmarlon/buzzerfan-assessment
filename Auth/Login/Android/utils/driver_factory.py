# Auth/Login/Android/utils/driver_factory.py
...
- Convert the relative APK path into an absolute path at runtime so the test is
  portable across machines (the required helper is ``resolve_apk_path``).
...
# Feature root = Auth/Login/Android (one level up from utils/).
...
# Repo root is three levels up: <repo>/Auth/Login/Android. Relative APK paths in
# credentials.json are interpreted from here.
